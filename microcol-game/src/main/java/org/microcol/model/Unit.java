...
public class Unit implements CargoHolder {
	...
	return new Unit(unit -> new Cargo(unit, unit.getType().getCargoCapacity(), unitPo.getCargo()), model,
		...
		UnitActionConverter.convert(unitPo.getAction()));
    }
	...
	Preconditions.checkState(isAtPlaceLocation(), "Unit have to be at map. Unit (%s) is at (%s)", this, place);
    }
	...
	Preconditions.checkState(availableMoves > 0, "There is not enough avilable moves (%s)", this);
	...
	final Colony col = model.getColonyAt(attackAt).orElseThrow(
		() -> new IllegalStateException(String.format("There are no units to figh of city at '%s'", attackAt)));
	...
	return MoreObjects.toStringHelper(this).add("id", id).add("type", type).add("owner", owner)
		.add("place", place == null ? place : place.getName()).add("availableMoves", availableMoves)
		.add("cargo", cargo).add("unitAction", unitAction).toString();
    }
	...
	Preconditions.checkState(getSupportedActions().contains(actionType), "Unsupported type '%s' for this unit '%s",
		unitAction.getType(), this);
	...
	if (getSupportedActions().contains(UnitActionType.plowField) && availableMoves > 0 && isAtPlaceLocation()) {
	    final TerrainType targetTerrain = model.getMap().getTerrainTypeAt(getLocation());
	    return TerrainType.GRASSLAND.equals(targetTerrain) || TerrainType.PRAIRIE.equals(targetTerrain)
