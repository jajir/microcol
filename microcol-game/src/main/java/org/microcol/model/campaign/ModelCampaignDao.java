...
public final class ModelCampaignDao {
	...
	public ModelMission loadFromFile(final String fileName, final MissionCallBack missionCallBack) {
		return makeFromModelPo(modelDao.loadModelFromFile(fileName), missionCallBack);
	}

	public ModelMission loadFromClassPath(final String fileName, final MissionCallBack missionCallBack) {
		return makeFromModelPo(modelDao.loadPredefinedModel(fileName), missionCallBack);
	}

	private ModelMission makeFromModelPo(final ModelPo modelPo, final MissionCallBack missionCallBack) {
		final Model model = Model.make(modelPo, MissionImpl.getPredefinedGameOverEvaluators());
		final Campaign campaign = campaignManager.getCampaignByName(resolve(modelPo.getCampaign().getName()));
		final CampaignMission campaignMission = campaign.getMisssionByName(modelPo.getCampaign().getMission());
		final Mission<?> mission = campaignMission.makeMission(missionCallBack, model, modelPo, campaignManager);
