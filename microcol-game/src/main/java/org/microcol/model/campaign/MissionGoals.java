...
public abstract class MissionGoals {
	...
	public <G extends MissionGoal> G getByClass(final Class<?> clazz) {
		Optional<MissionGoal> oGoal = goals.stream().filter(goal -> goal.getClass().equals(clazz)).findFirst();
		if (oGoal.isPresent()) {
			return (G) oGoal.get();
		} else {
			throw new InvalidParameterException(String.format("There is goal defined by class '%s'", clazz.getName()));
		}
	}
